...
def get_unique_palette(num_categories):
    base_corporate = [PRIMARY_COLORS["Electric Blue"], PRIMARY_COLORS["Serene Blue"]] + ACCENT_COLORS
    if num_categories <= len(base_corporate):
        return base_corporate[:num_categories]
    overflow_needed = num_categories - len(base_corporate)
    supplementary_colors = sns.color_palette("Spectral", overflow_needed).as_hex()
    return base_corporate + supplementary_colors
...
def apply_fractional_allocation(df, col_type, specific_mapping=None):
    ...
    if col_type == "Assessment Type":
    ...
    elif col_type in ["CPC 1-digit", "HS 2-digit"]:
        # Map string prefixes
        target_col = "Sector: CPC 3-digit (v2.1)" if col_type == "CPC 1-digit" else "Product: HS 6-digit (2022)"
        def map_codes(val):
            val = str(val).strip()
            if val.upper() in ["NAN", "NONE", ""]: return ["Other Products"]
            tokens = [t.strip() for t in val.split(",") if t.strip()]
            matched = set()
            for token in tokens:
                prefix = token[:1] if col_type == "CPC 1-digit" else token[:2].zfill(2)
                if prefix in specific_mapping: matched.add(specific_mapping[prefix])
            return list(matched) if matched else ["Other Products"]
            
        df_temp["Active_Categories"] = df_temp[target_col].apply(map_codes)
        df_temp["Denominator"] = df_temp["Active_Categories"].apply(len)
        
    else: # Boolean lists (Sectors, Motives, Policies)
        ...
        def get_active(row):
            ...
            return active if active else [f"Other {col_type}s"]
        df_temp["Active_Categories"] = df_temp.apply(get_active, axis=1)
        df_temp["Denominator"] = df_temp["True_Count"].replace(0, 1)

    # Fractional Math
    df_temp["Allocated_USD"] = df_temp["Total_USD_Value"] / df_temp["Denominator"]
...
if uploaded_file is not None:
    ...
    # Handle affected jurisdictions splitting for filter
    ...
    # 3. Assessment
    assessments = st.sidebar.multiselect("Initial Assessment", ["Liberalising", "Distortive", "Other"], default=[])
    ...
    with tab1:
        ...
        display_cols = ["Title", "Announcement Date", "Implementing Jurisdiction", "Affected Jurisdiction", 
                        "Initial Assessment", "Trade Covered (USD Million)", "Size of Subsidy (USD Million)", "URL"]
    ...
    with tab2:
        ...
        with col1:
            disaggregation = st.selectbox("Disaggregate By", 
                                          ["Sector", "Motive", "Policy Instrument", "Assessment Type", "CPC 1-digit"])
        with col2:
            freq_map = {"Yearly": "Y", "Quarterly": "Q", "Monthly": "M", "Daily": "D"}
            freq_choice = st.selectbox("Time Frequency", list(freq_map.keys()), index=0)
            freq_code = freq_map[freq_choice]
        with col3:
            smoothing = st.slider("Moving Average (Periods)", min_value=1, max_value=24, value=1, 
                                  help="Smooths the data over X periods based on the chosen frequency.")
        with col4:
            metric_choice = st.selectbox("Metric to Plot", ["Combined USD Value", "Policy Count"])
            metric_col = "Allocated_USD" if metric_choice == "Combined USD Value" else "Allocated_Count"
        ...
        if st.button("Generate Plot", type="primary"):
            with st.spinner("Applying fractional allocations and rendering..."):
                ...
                # Ensure continuous timeline indexing
                ...
                colors = get_unique_palette(len(categories))
                ...
                # Dynamic X-ticks to avoid overcrowding
